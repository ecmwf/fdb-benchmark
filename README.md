...
artifact_dir=$install_dir/fdb-hammer-parallel/artifacts
...
hammer_args="--nodelist $WRITERS --ppn $writeppn \
...
```bash
# To enable consistency checks change the hammer_args to the following:
hammer_args="--nodelist $WRITERS --ppn $writeppn \
    --nodelist-read $READERS --ppn-read $readppn --read-nodes-per-step $rnpm \
    --nmembers $members \
    --root $build_root --config $build_root/config.yaml.in \
